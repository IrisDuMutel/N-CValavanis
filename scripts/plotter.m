...
y_real = out.state(:,2,:);
z_real = out.state(:,3,:);
phi_real = out.state(:,4,:);
the_real = out.state(:,5,:);
psi_real = out.state(:,6,:);
u_real = out.state(:,7,:);
v_real = out.state(:,8,:);
w_real = out.state(:,9,:);
...
hold on
plot3(x_real(1,:),y_real(1,:),-z_real(1,:))
grid on
...
hold on
plot(out.time(:,1),x_real(1,:),'lineWidth',1.5)
grid on
ylabel(['X [m]'])
t = title('Position');
...
hold on
plot(out.time(:,1),y_real(1,:),'lineWidth',1.5)
grid on
ylabel(['Y [m]'])
legend('Reference', 'Real')
subplot(3,1,3)
...
hold on
plot(out.time(:,1),-z_real(1,:),'lineWidth',1.5)
grid on
ylabel(['Z [m]'])
...
hold on
plot(out.time(:,1),phi_real(1,:),'lineWidth',1.5)
grid on
ylabel(['\phi [rad]'])
t = title('Attitude');
set(t, 'horizontalAlignment', 'center')
...
hold on
plot(out.time(:,1),the_real(1,:),'lineWidth',1.5)
grid on
ylabel(['\theta [rad]'])
legend('Reference', 'Real')
subplot(3,1,3)
...
hold on
plot(out.time(:,1),psi_real(1,:),'lineWidth',1.5)
grid on
ylim([-10^-3, 10^-3])
ylabel(['\psi [rad]'])
...
hold on
plot(out.time(:,1),u_real(1,:),'lineWidth',1.5)
grid on
ylabel(['u [m/s]'])
t = title('Velocity');
set(t, 'horizontalAlignment', 'center')
...
hold on
plot(out.time(:,1),v_real(1,:),'lineWidth',1.5)
grid on
ylabel(['v [m/s]'])
legend('Reference', 'Real')
subplot(3,1,3)
...
hold on
plot(out.time(:,1),w_real(1,:),'lineWidth',1.5)
grid on
ylabel(['\w [m/s]'])
